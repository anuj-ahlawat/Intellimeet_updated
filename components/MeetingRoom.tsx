...
const MeetingRoom = () => {
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden pt-4 text-white">
      ...
      {/* Captions */}
      <LiveCaptions isVisible={showCaptions} onToggle={() => setShowCaptions(!showCaptions)} />

      {/* Chat */}
      <ChatPanel isOpen={showChat} onClose={() => setShowChat(false)} call={call} />
      ...
      <div className="fixed bottom-0 flex w-full items-center justify-center gap-3 px-4 pb-3">
        ...
        <button
          ...
          className={cn(
            'cursor-pointer rounded-2xl px-4 py-2 flex items-center gap-2 transition-colors',
            showCaptions ? 'bg-green-500/20 text-green-400' : 'bg-[#19232d] hover:bg-[#4c535b]'
          )}
        >
          <Mic size={20} className={cn(showCaptions ? 'text-green-400' : 'text-white')} />
          <span className="hidden md:inline text-sm">Captions</span>
        </button>
